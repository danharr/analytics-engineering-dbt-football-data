-- Top 5 goal scorers for the 2022-23 season, resolving player names from the
-- players seed (populated via the players-by-id API) so the API is only hit when
-- new players need names. Backs the "Season Reviews — 2022-23" page.
copy (
    with goals as (
        select
            player_id,
...
to 'assets/data/top_scorers_2022_23.csv' (header, delimiter ',')