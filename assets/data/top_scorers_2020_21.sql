-- Top 5 goal scorers for the 2020-21 season, resolving player names from the
-- players seed (populated via the players-by-id API) so the API is only hit when
-- new players need names. Backs the "Season Reviews — 2020-21" page.
copy (
    with goals as (
        select
            player_id,
...
to 'assets/data/top_scorers_2020_21.csv' (header, delimiter ',')