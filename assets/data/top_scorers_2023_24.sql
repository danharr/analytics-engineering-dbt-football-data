-- Top 5 goal scorers for the 2023-24 season, resolving player names from the
-- players seed (populated via the players-by-id API) so the API is only hit when
-- new players need names. Backs the "Season Reviews — 2023-24" page.
copy (
    with goals as (
        select
            player_id,
...
to 'assets/data/top_scorers_2023_24.csv' (header, delimiter ',')