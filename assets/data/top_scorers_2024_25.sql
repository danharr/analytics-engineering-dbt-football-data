-- Top 5 goal scorers for the 2024-25 season, resolving player names from the
-- players seed (populated via the players-by-id API) so the API is only hit when
-- new players need names. Backs the "Season Reviews — 2024-25" page.
copy (
    with goals as (
        select
            player_id,
...
to 'assets/data/top_scorers_2024_25.csv' (header, delimiter ',')