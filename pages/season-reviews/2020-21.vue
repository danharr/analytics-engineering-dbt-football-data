  ...
  <v-row>
    <v-col cols="12" md="10" offset-md="1">
      <v-card color="secondary" variant="tonal">
        <v-card-title>
          ...
          2020-21 Season Summary In Charts
        </v-card-title>
        <v-card-subtitle>
        ...
        <v-card-text class="pt-0">
          <p class="mb-0">
            The 2020-21 season in charts. Substitutions, yellow cards and red cards are
    ...
    <v-col cols="12" md="10" offset-md="1">
      <v-card>
        ...
        <v-card-text>
          ...
          <SubTimingChart :data="subTiming2020_21" />
        </v-card-text>
      </v-card>
    </v-col>
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-table density="comfortable">
            ...
            <tbody>
              <tr v-for="row in topScorers2020_21" :key="row.rank">
                <td>{{ row.rank }}</td>
    ...
    <v-col cols="12" md="6" offset-md="3">
      <v-card>
        ...
        <v-card-subtitle>
          2020-21 most productive assist-scorer pairings
        </v-card-subtitle>
        <v-card-text>
          ...
          <v-table density="comfortable">
            ...
            <tbody>
              <tr v-for="row in partnerships2020_21" :key="row.rank">
                <td>{{ row.rank }}</td>
...
<script setup>
import { subTiming2020_21, topScorers2020_21, partnerships2020_21, yellowCards, redCards, joinNames, mostAndFewest, fewestSummary, datasetLd } from '~/composables/useData'

const yellows = yellowCards.filter(r => r.season_label === '2020-21')
const reds = redCards.filter(r => r.season_label === '2020-21')

const teams = [...new Set(subTiming2020_21.map(r => r.team_name))]

const subSummary = (() => {
  const { most } = mostAndFewest(subTiming2020_21, teams)
...
const scorerSummary = `${topScorers2020_21[0].player_name} won the Golden Boot with ${topScorers2020_21[0].goals} goals.`

const bestPair = partnerships2020_21[0]
...
useHead({
  title: '2020-21 Season Summary In Charts',
  meta: [
    {
      name: 'description',
      content: 'The 2020-21 Premier League season in charts: substitution, yellow card and red card timing heat maps plus the season\'s top five scorers and assist-scorer partnerships, generated from the per-match event feed.'
    }
  ],
  script: [
    {
      ...
      innerHTML: JSON.stringify(datasetLd({
        name: 'Premier League 2020-21 Season Summary Dataset',
        description: 'The 2020-21 Premier League season in charts: substitutions, yellow and red cards by minute, plus the top five scorers and assist-scorer partnerships, from the per-match event feed.',
        path: '/season-reviews/2020-21',
        csv: 'sub_timing_2020_21.csv',
        keywords: ['substitutions', '2020-21', 'Premier League']
      }))
    }
  ]
})
