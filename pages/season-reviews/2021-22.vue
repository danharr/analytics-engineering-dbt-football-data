  ...
  <v-row>
    <v-col cols="12" md="10" offset-md="1">
      <v-card color="secondary" variant="tonal">
        <v-card-title>
          ...
          2021-22 Season Summary In Charts
        </v-card-title>
        <v-card-subtitle>
        ...
        <v-card-text class="pt-0">
          <p class="mb-0">
            The 2021-22 season in charts. Substitutions, yellow cards and red cards are
    ...
    <v-col cols="12" md="10" offset-md="1">
      <v-card>
        ...
        <v-card-text>
          ...
          <SubTimingChart :data="subTiming2021_22" />
        </v-card-text>
      </v-card>
    </v-col>
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-table density="comfortable">
            ...
            <tbody>
              <tr v-for="row in topScorers2021_22" :key="row.rank">
                <td>{{ row.rank }}</td>
    ...
    <v-col cols="12" md="6" offset-md="3">
      <v-card>
        ...
        <v-card-subtitle>
          2021-22 most productive assist-scorer pairings
        </v-card-subtitle>
        <v-card-text>
          ...
          <v-table density="comfortable">
            ...
            <tbody>
              <tr v-for="row in partnerships2021_22" :key="row.rank">
                <td>{{ row.rank }}</td>
...
<script setup>
import { subTiming2021_22, topScorers2021_22, partnerships2021_22, yellowCards, redCards, joinNames, mostAndFewest, fewestSummary, datasetLd } from '~/composables/useData'

const yellows = yellowCards.filter(r => r.season_label === '2021-22')
const reds = redCards.filter(r => r.season_label === '2021-22')

const teams = [...new Set(subTiming2021_22.map(r => r.team_name))]

const subSummary = (() => {
  const { most } = mostAndFewest(subTiming2021_22, teams)
...
const scorerSummary = 'Mohamed Salah and Son Heung-min shared the Golden Boot with 23 goals each.'

const bestPair = partnerships2021_22[0]
...
useHead({
  title: '2021-22 Season Summary In Charts',
  meta: [
    {
      name: 'description',
      content: 'The 2021-22 Premier League season in charts: substitution, yellow card and red card timing heat maps plus the season\'s top five scorers and assist-scorer partnerships, generated from the per-match event feed.'
    }
  ],
  script: [
    {
      ...
      innerHTML: JSON.stringify(datasetLd({
        name: 'Premier League 2021-22 Season Summary Dataset',
        description: 'The 2021-22 Premier League season in charts: substitutions, yellow and red cards by minute, plus the top five scorers and assist-scorer partnerships, from the per-match event feed.',
        path: '/season-reviews/2021-22',
        csv: 'sub_timing_2021_22.csv',
        keywords: ['substitutions', '2021-22', 'Premier League']
      }))
    }
  ]
})
