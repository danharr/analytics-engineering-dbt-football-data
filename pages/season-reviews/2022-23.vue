  ...
  <v-row>
    <v-col cols="12" md="10" offset-md="1">
      <v-card color="secondary" variant="tonal">
        <v-card-title>
          ...
          2022-23 Season Summary In Charts
        </v-card-title>
        <v-card-subtitle>
        ...
        <v-card-text class="pt-0">
          <p class="mb-0">
            The 2022-23 season in charts. Substitutions, yellow cards and red cards are
    ...
    <v-col cols="12" md="10" offset-md="1">
      <v-card>
        ...
        <v-card-text>
          ...
          <SubTimingChart :data="subTiming2022_23" />
        </v-card-text>
      </v-card>
    </v-col>
      ...
      <v-card>
        ...
        <v-card-text>
          ...
          <v-table density="comfortable">
            ...
            <tbody>
              <tr v-for="row in topScorers2022_23" :key="row.rank">
                <td>{{ row.rank }}</td>
    ...
    <v-col cols="12" md="6" offset-md="3">
      <v-card>
        ...
        <v-card-subtitle>
          2022-23 most productive assist-scorer pairings
        </v-card-subtitle>
        <v-card-text>
          ...
          <v-table density="comfortable">
            ...
            <tbody>
              <tr v-for="row in partnerships2022_23" :key="row.rank">
                <td>{{ row.rank }}</td>
...
const teams = [...new Set(subTiming2022_23.map(r => r.team_name))]

const subSummary = (() => {
  const { most } = mostAndFewest(subTiming2022_23, teams)
...
const scorerSummary = `${topScorers2022_23[0].player_name} won the Golden Boot with ${topScorers2022_23[0].goals} goals.`

const bestPair = partnerships2022_23[0]
...
useHead({
  title: '2022-23 Season Summary In Charts',
  meta: [
    {
      name: 'description',
      content: 'The 2022-23 Premier League season in charts: substitution, yellow card and red card timing heat maps plus the season\'s top five scorers and assist-scorer partnerships, generated from the per-match event feed.'
    }
  ],
  script: [
    {
      ...
      innerHTML: JSON.stringify(datasetLd({
        name: 'Premier League 2022-23 Season Summary Dataset',
        description: 'The 2022-23 Premier League season in charts: substitutions, yellow and red cards by minute, plus the top five scorers and assist-scorer partnerships, from the per-match event feed.',
        path: '/season-reviews/2022-23',
        csv: 'sub_timing_2022_23.csv',
        keywords: ['substitutions', '2022-23', 'Premier League']
      }))
    }
  ]
})
