...
const props = defineProps({
  ...
  color: { type: String, default: '#b71c1c' },
  itemLabel: { type: String, default: 'substitution' }
})
...
function renderChart() {
  ...
  // One 90-bin row per team: counts[minute-1] = number of events at that minute.
  ...
  const color = d3.scaleSequential()
    .domain([0, maxCount])
    .interpolator(t => d3.interpolateRgb('#ffffff', props.color)(t))
  ...
  legend.append('text').attr('x', legW + 8).attr('y', 6).attr('font-size', '10px').attr('fill', '#666').text(props.itemLabel + 's')
  ...
  chart.selectAll('rect.cell')
    .on('mousemove', function (event, d) {
      tooltip
        .style('opacity', 1)
        .html(`<strong>${d.team}</strong><br>Minute ${d.minute}: ${d.n} ${props.itemLabel}${d.n === 1 ? '' : 's'}`)
